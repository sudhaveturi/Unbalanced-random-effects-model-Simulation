...
#nIter = 1000
num.reps = 100
pval = rep(list(list()),1)
pval_bal = pval_mod = pval_ext = list()
...
pval_final = list(pval_n1,pval_n2)
  ...
  for(j in 1:length(n))
  {
    names(pval_final) = c("N=100","N=500")
    for(k in 1:length(m))
    {
      names(pval_final[[j]]) = c("m=2","m=5","m=10")
      for(l in 1:length(bal))
      {   
        names(pval_final[[j]][[k]]) = c("Balanced","Moderate","Extreme")
      }
    }
  }
...
#        {      
 ...
 #       fixed_alt <- lm(yA ~ a)
        ...
        pv[t,2] = as.numeric(unlist(summary(aov(yA ~ a)))[9])
 
    }
        pval[[i]] = pv
        print(paste("N(j)=",n[j],"m(w)=",m[w],"k=",bal[k],"iter=",i,sep=" "))
 #       }
...
save(pval_final,file=paste("/home/sveturi/BST760/Type1Error/Pval_Type1Error_",i,".rda",sep=""))
#save(pval_final, file=paste("C:/Users/Guha/Dropbox/UAB/Coursework/Spring2014/BST760/BST760_Project/Code/Pval_Type1Error_",i,".rda",sep=""))
